...
class ResponseRecomendApp:
    ...
    def __recvQueueDeque(self, msg):
        self.__recv_queue.get(msg)
        
    
    def __sendQueueEnque(self,msg):
        '''
        1. sendQueue에 메세지를 집어넣음.
        '''
        self.__send_queue.put(msg)
    
    def __isEmptySendQueue(self):
        '''
    ...
    def __sendQueueDeque(self, msg):
        '''
        1. sendQueue가 비어있지 않다면 진행
        2. dict메세지를 전송?한다.
        '''
        if self.__send_queue.__isEmptySendQueue():
            print('큐가 비어있음')
        else:

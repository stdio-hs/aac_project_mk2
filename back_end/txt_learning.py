...
# for tag in set(labels_train):
#     patterns = [data_train[i] for i in range(len(data_train)) if labels_train[i] == tag]
#     train_data['intence'].append({'tag': tag, 'patterns': patterns, 'response': []})

# for tag in set(labels_test):
#     patterns = [data_test[i] for i in range(len(data_test)) if labels_test[i] == tag]
#     test_data['intence'].append({'tag': tag, 'patterns': patterns, 'response': []})

# # train과 test 데이터를 JSON 파일로 저장
